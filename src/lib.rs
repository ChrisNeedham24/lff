...
pub trait LffPrinter {
    /// Prints the given `Display`-able value - we maintain a reference to `self` so that the test
    /// implementations of this trait can supply data structures to keep track of passed values.
    fn println(&mut self, value: impl Display);
}

/// The standard printer, printing straight to standard out.
pub struct LffStdoutPrinter;
...
impl LffPrinter for LffStdoutPrinter {
    /// Prints the given `Display`-able value using the `println!` macro.
    #[cfg(not(tarpaulin_include))]
    fn println(&mut self, value: impl Display) {
        println!("{}", value);
    }
...
fn handle_entry(file_path: PathBuf, args: &LffArgs) -> Result<LffFile> {
    ...
    let file_name: OsString = match args.absolute {
        true => fs::canonicalize(&file_path)
    ...
    let file_size: u64 = fs::symlink_metadata(&file_path)
...
pub fn run_finder(args: LffArgs, printer: &mut impl LffPrinter) -> Result<()> {
    let directory: ReadDir = fs::read_dir(&args.directory)
    ...
    if !files_vec.is_empty() {
        ...
        for file in &files_vec {
            if args.extension.is_some() || args.name_pattern.is_some() {
                if let Some(unicode_name) =  file.name.to_str() {
                    let ext_idx: usize =
                        unicode_name.len() - args.extension.clone().unwrap().len() - 1;
                    let name_no_ext: &str = &unicode_name[..ext_idx];
                    let coloured_ext: ColoredString = unicode_name[ext_idx..].red();
        ...
        printer.println(String::from(NO_FILES_FOUND_STR).red());
    }
...
mod tests {
    ...
    fn test_lff_eyre_handler() {
        ...
        let test_dir: ReadDir = fs::read_dir("test_resources").unwrap();
    ...
    fn test_hidden_paths() {
        ...
        unsafe {
            ...
            let non_utf8_path: &Path = Path::new(str::from_utf8_unchecked(&invalid_bytes));
    ...
    fn test_run_finder_invalid_dir() {
        ...
        let dir_err: Report = run_finder(test_args, &mut LffTestPrinter::default()).unwrap_err();
        assert_eq!(
